...
async function getAudioStream(message, username, accessToken, audioWorkletNode) {
    ...
    const response = await fetch('https://lg5m7pmkstz3ims7qkmh7u4xfi0gjebf.lambda-url.us-east-1.on.aws/', {
        method: 'POST',
        headers: {
    ...
    while (true) {
        ...
        audioWorkletNode.port.postMessage({ method: 'buffer', args: { channelData: [float32Array], sampleRate: 48000 }});
    }
}
...
function convertAndNormalizeToFloat32Array(value) {
    ...
    let int16Array;
    try {
         int16Array = new Int16Array(value.buffer, value.byteOffset, value.byteLength / Int16Array.BYTES_PER_ELEMENT);
    } catch (error) {
